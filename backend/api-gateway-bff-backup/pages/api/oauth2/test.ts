...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const authServiceUrl = (process.env.AUTH_SERVICE_URL as string) || 'http://authentication-identity-service:8000'
...
export default async function handler(req: any, res: any) {
  ...
  const authServiceUrl = ((globalThis as any).process?.env?.AUTH_SERVICE_URL as string) || 'http://authentication-identity-service:8000'
